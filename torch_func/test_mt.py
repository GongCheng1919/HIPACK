...
def main():
    ...
    # Formula: FLOPS = (2 * Cin * K * K * Hout * Wout * Cout * batch_size) / time_in_seconds
    output_height = ((args.height + 2 * args.padding - args.kernel_size) // args.stride) + 1
    output_width = ((args.width + 2 * args.padding - args.kernel_size) // args.stride) + 1
    
    flop_count = 2 * args.in_channels * args.kernel_size * args.kernel_size * output_height * output_width * args.out_channels * args.batch_size
    
    direct_flops = flop_count / (time_direct / 1000)  # Convert ms to seconds
    conv_flops = flop_count / (time_conv / 1000)
    qconv_flops = flop_count / (time_qconv / 1000)
    ...
    print("\nPerformance (GFLOPS):")
    print("DirectConv2d:", direct_flops / 1e9)
    print("nn.Conv2d:   ", conv_flops / 1e9)
    print("q.Conv:      ", qconv_flops / 1e9)
