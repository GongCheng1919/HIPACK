...
def generate_report(filename: str):
    ...
    if "hipack" not in df.columns:
        print("No data for hipack, please check your runnings")
        return
    if "qnnpack" not in df.columns:
        print("No data for qnnpack, please check your runnings")
        return
    if "float32" not in df.columns:
        print("No data for float32, please check your runnings")
        return
    ...
    df["winner"] = df.apply(lambda row: "hipack_ms_avg" if row["hipack_ms_avg"] < row["float32_ms_avg"] and row["hipack_ms_avg"] < row["qnnpack_ms_avg"] else ("qnnpack_ms_avg" if row["qnnpack_ms_avg"] < row["float32_ms_avg"] else "float32_ms_avg"), axis=1)

    df.to_excel(f"{filename}.xlsx", index=True)
    print(f"Report saved to {filename}.xlsx")